...
exports.finalizeTest = exports.submitRecording = exports.submitAnswer = exports.getNextQuestion = exports.initializeTest = void 0;
...
function getTestSessionCollection() {
    return database_1.client.db("aegis").collection("testSessions");
}
function getTestResultCollection() {
    return database_1.client.db("aegis").collection("testResults");
}
// Calculate adaptive difficulty based on performance
function calculateNextDifficulty(currentDifficulty, isCorrect) {
...
async function initializeTest(req, res) {
    try {
        const { type } = req.body;
        if (type === 'practice') {
...
async function getNextQuestion(req, res) {
    try {
        ...
        const sessionCol = getTestSessionCollection();
        const session = await sessionCol.findOne({ _id: new mongodb_1.ObjectId(sessionId) });
        if (!session || session.status !== 'in-progress') {
            return res.status(400).json({ error: 'Invalid session' });
        }
        if (session.incorrectAnswers >= 5) {
            session.status = 'completed';
            await sessionCol.updateOne({ _id: new mongodb_1.ObjectId(sessionId) }, { $set: { status: 'completed' } });
...
async function submitAnswer(req, res) {
    try {
        ...
        const questionCol = (0, Question_1.getQuestionCollection)();
        const session = await sessionCol.findOne({ _id: new mongodb_1.ObjectId(sessionId) });
        const question = await questionCol.findOne({ _id: new mongodb_1.ObjectId(questionId) });
        if (!session || !question) {
        ...
        session.questions.push({
            questionId: new mongodb_1.ObjectId(questionId),
            userAnswer: answer,
            timeSpent,
            difficulty: question.difficulty
        });
        session.currentScore = calculateScore(session.questions);
        await sessionCol.updateOne({ _id: new mongodb_1.ObjectId(sessionId) }, {
            $set: {
        ...
        await questionCol.updateOne({ _id: new mongodb_1.ObjectId(questionId) }, {
            $inc: { timesSeen: 1 },
            $set: {
...
async function submitRecording(req, res) {
    try {
        const { sessionId } = req.params;
        const { recordingUrl } = req.body;
        const sessionCol = getTestSessionCollection();
        await sessionCol.updateOne({ _id: new mongodb_1.ObjectId(sessionId) }, { $set: { recordingUrl } });
...
exports.submitRecording = submitRecording;
async function finalizeTest(req, res) {
    try {
        const { sessionId } = req.params;
        const sessionCol = getTestSessionCollection();
        ...
        const questions = await questionCol.find({
            _id: { $in: session.questions.map(q => new mongodb_1.ObjectId(q.questionId)) }
        }).toArray();
        ...
        const testResult = {
            sessionId: new mongodb_1.ObjectId(sessionId),
            userId: session.userId,
            finalScore: session.currentScore,
            questionBreakdown: Object.entries(categoryBreakdown).map(([category, stats]) => ({
                category,
        ...
        await resultCol.updateOne({ _id: testResult._id }, { $set: { percentile: testResult.percentile } });
...
exports.finalizeTest = finalizeTest;
